...
def generate_dtrace(project):
    common.run_dljc(project, ['dyntrace'], ['--cache', '--daikon-xml'])
...
def generate_project_kernel(project, cluster_json=None):
    ...
    if cluster_json:
        graph_kernel_cmd = [
            'python3',
            common.get_simprog('precompute_kernel.py'), project_dir, kernel_file_path, cluster_json
        ]
        common.run_cmd(graph_kernel_cmd, 'graphkernel')
    else:
        graph_kernel_cmd = [
            'python3',
            common.get_simprog('precompute_kernel.py'), project_dir, kernel_file_path
        ]
        common.run_cmd(graph_kernel_cmd, 'graphkernel')
...
def compute_clusters_for_classes(project_list, out_file_name, cf_map_file_name, wf_map_file_name):
    ...
    clusterer_cmd = [
        'java', '-jar',
        common.get_jar('clusterer.jar'), '-cs', '3', '-out', out_file_name, '-cfm',
        cf_map_file_name, '-wfm', wf_map_file_name, '-dirs'
    ]
