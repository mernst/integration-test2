...
def generate_graphs(project):
  ...
  """
    ...
    common.run_dljc(project, ['graphtool'],
                    ['--graph-jar', common.get_jar('prog2dfg.jar'), '--cache'])
        ...
        for project in projects:
            ...
            if os.path.isfile(project_kernel_file_path):
                ...
                print("No kernel file find for project {0}.\n   {1} is not a file.".format(
                    project, project_kernel_file_path))
                sys.exit(0)
...
def generate_project_kernel(project, cluster_json=None):
    ...
    if cluster_json:
        ...
        graph_kernel_cmd = [
            ...
            os.path.join(dot.dot_dir(project), out_dir), kernel_file_path, cluster_json
        ]
        ...
        graph_kernel_cmd = [
            'python3',
            common.get_simprog('precompute_kernel.py'),
            os.path.join(dot.dot_dir(project), out_dir), kernel_file_path
        ]
        common.run_cmd(graph_kernel_cmd, True)
    ...
    with open(result_file, "w") as fo:
        for dot_file in this_method_map.keys():
            ...
            result_program_list_with_score = sim.find_top_k_similar_graphs(
                dot_file, dot_file, top_k, iter_num, cluster_json)
            line = dot_file + ":\n"
...
def main():
    ...
    parser.add_argument(
        ...
        "--cluster",
        type=str,
        help="path to the input json file that contains the clustering information.")
    parser.add_argument("-d", "--dir", type=str, required=True, help="output folder.")
    parser.add_argument("-p",
                        "--plist",
                        type=str,
                        help="a comma separated list of projects to work with.")
    ...
    parser.add_argument(
        ...
        "--sim",
        type=str,
        help="specify a subset of projects for finding similar programs in the list of projects.")
    ...
    if args.sim:
        ...
        for project in project_sublist:
            if project not in project_list:
                ...
                check_similarity(project, result_file, kernel_file, dot_method_map, json_file,
                                 args.cluster, min(5, len(project_list)))
    else:
        for project in project_list:
