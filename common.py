...
def run_cmd(cmd, output=False, timeout=None):
    ...
    try:
        ...
        output('calling {} failed\n{}'.format(friendly_cmd, traceback.format_exc()))
...
def get_class_dirs(project_name):
    ...
    dljc_output = os.path.join(get_project_dir(project_name), DLJC_OUTPUT_DIR, 'javac.json')
...
def copy_dyntrace_files(project_name):
    ...
    for addon in addons:
        ...
        shutil.copyfile(os.path.join(DYNTRACE_ADDONS_DIR, addon), os.path.join(out_dir, addon_type))
...
def run_dljc(project_name, tools=[], options=[]):
    ...
    if tools:
        if skipped_tools:
            print("Running {} on {} (skipping {})".format(', '.join(remaining_tools), project_name,
                                                          ', '.join(skipped_tools)))
        else:
            print("Running {} on {}".format(', '.join(remaining_tools), project_name))
    ...
    with cd(project_dir):
        ...
        dljc_command = [
            DLJC_BINARY, '-l', LIBS_DIR, '-o', DLJC_OUTPUT_DIR, '--timeout',
            str(timelimit)
        ]
...
def ensure_java_home():
    if not os.environ.get('JAVA_HOME'):
        ...
        if os.path.exists('/usr/libexec/java_home'):
            ...
            print(
                "ERROR: {} requires the JAVA_HOME environment variable to be set, and we couldn't set it automatically. Please set the JAVA_HOME environment variable and try again."
                .format(caller))
            sys.exit(0)
...
def setup_checker_framework_env():
    ...
    os.environ[get_os_lib_path_name()] = os.path.join(TOOLS_DIR, 'checker-framework-inference',
                                                      'lib')
...
def recompile_checker_framework():
  ...
  """
    ...
    with cd(checker_framework_inference_dir):
        ...
        run_cmd("./gradlew dist -i", 'checker_build')

    with cd(ontology_dir):
        run_cmd("./gradlew build -i -x test", 'checker_build')
