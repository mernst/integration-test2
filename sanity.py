...
def error(project, check, message):
    ...
    print('ERROR: [{proj}] - {check} - {msg}'.format(proj=project['name'], check=check,
                                                     msg=message))
...
def test_src_dirs(project):
    return [entry for entry in os.listdir(project_dir(project)) if 'test-src' in entry]
    ...
    with open(randoop_log) as f:
        for line in f.readlines():
            if "Randoop failed." in line:
                errors.extend(
                    error(project, 'randoop', 'Randoop failed. See randoop.log:{}'.format(lineno)))
            lineno += 1
