...
for r, ds, fs in os.walk(repo_dir):
    for f in fnmatch.filter(fs, '*.dot'):
        ...
        if len(sys.argv) == 4:
            ...
            print("Relabeled {0} out of {1} nodes in {2}.".format(relabel_count,
                                                                  gk.g.number_of_nodes(),
                                                                  gk.dot_file))
        gk.init_wl_kernel()
        wls = gk.compute_wl_kernel(num_iter)
        wl_str = "###".join(
            [";;;".join([",,,".join([str(x), str(y)]) for (x, y) in wl]) for wl in wls])
        fo.write(
            os.path.join(
...
print("\n\nIn total, relabeled {0} nodes out of {1} nodes in {2}.\n".format(
    total_relabel_count, total_node_count, repo_dir))
