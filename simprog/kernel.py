...
class GraphKernel(object):
            ...
            if 'label' in self.g.node[n]:  # some node may not have any label!
                ...
                if quote_free in label_map:
                    ...
                    if len(label_map[quote_free]) > 0:  # may have 0 label
                        print('Relabeled {0} to {1} in {2}.'.format(quote_free,
                                                                    label_map[quote_free][0],
                                                                    self.dot_file))
                        count += 1
    ...
    def compute_wl_kernel(self, num_iter=3, ignore_edge_label=True):
        ...
        for i in range(num_iter):
            wl = [(x, y * weight_vector[i])
                  for (x, y) in self.compute_wl_label(i, ignore_edge_label)]
            wl_vector.append(wl)
    ...
    def compute_wl_node_label(self, node, prev_iter, ignore_edge_label=True):
        ...
        if ignore_edge_label:
            ...
            pred_labels = [
                self.g.node[p][wl_label_key] + ',' + self.g.edge[p][node]['label'] for p in preds
            ]
            succ_labels = [
                self.g.node[s][wl_label_key] + ',' + self.g.edge[node][s]['label'] for s in succs
            ]
        pred_labels.sort()
        succ_labels.sort()
