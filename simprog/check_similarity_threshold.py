...
def check_all(repo_kernel_file, threshold_list, top_k):
    ...
    with open(repo_kernel_file, 'r') as fi:
        for line in fi:
            ...
            parts = line.split('\t')
            dot_file = parts[0]
            result_program_list_with_score = sim.find_top_k_similar_graphs(dot_file, 'g', top_k,
                                                                           3)  # num_iter = 3
            ...
            for (i, threshold) in enumerate(threshold_list):
                ...
                if cr == 'tp':
                    ...
                    tn_list[i] += 1
                acc = check_top_k_result(true_prob, result_program_list_with_score, threshold,
                                         top_k)
                acc_list[i] += acc
...
def main():
    ...
    total, tp_list, fp_list, tn_list, fn_list, acc_list = check_all(repo_kernel_file,
                                                                    threshold_list, top_k)
