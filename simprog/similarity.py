...
class Similarity(object):
    ...
    def compute_wl_kernel_matrix(self, num_iter=3):
        ...
        for (x, y) in itertools.product(range(graph_num), range(graph_num)):
            if x < y:  # only need upper-triangular portion since the kernel matrix is symmetric
                kernel_matrix[x][y] = self.compute_wl_kernel_scalar_product(
                    self.wl_vectors[self.graphs[x]], self.wl_vectors[self.graphs[y]], num_iter)
    ...
    def compute_similarity_between_vectors_old(self, wl1, gs1, wl2, gs2, num_iter, sym=True):
        ...
        if gs1 < gs2:
            similarity_score = float(self.compute_wl_kernel_scalar_product(
                wl1, wl2, num_iter)) * (float(gs1) / (gs2 + gs_diff * num_iter)) / self_score1
        else:
            if sym:  # score(wl1, wl2) = score(wl2, wl1)
                self_score2 = self.compute_wl_kernel_scalar_product(wl2, wl2, num_iter)
                similarity_score = float(self.compute_wl_kernel_scalar_product(
                    wl1, wl2, num_iter)) * (float(gs2) / (gs1 + gs_diff * num_iter)) / self_score2
            else:
                similarity_score = float(elf.compute_wl_kernel_scalar_product(
                    wl1, wl2, num_iter)) / self_score1
    ...
    def compute_similarity_between_vectors(self, wl1, wl2, num_iter):
        ...
        for i in range(num_iter + 1):
            try:
                wl_kernel += ratio_list[i] * self.compute_vector_scalar_product(wl1[i], wl2[i]) / (
                    self.compute_vector_2norm(wl1[i]) * self.compute_vector_2norm(wl2[i]))
            except IndexError:
                print(len(wl1))
                print(len(wl2))
                sys.exit(1)
    ...
    def find_top_k_similar_graphs(self, graph_dot_file, graph_name, k, num_iter, cluster_json=None):
        ...
        wl_pairs.sort(key=lambda x: x[1], reverse=True)
        if k > len(wl_pairs):
            logging.warning("Trying to select {0} programs out of only {1} programs.".format(
                k, len(wl_pairs)))
    ...
    def compute_wl_angle(self, wl1, wl2, num_iter):
        ...
        return float(self.compute_wl_kernel_scalar_product(wl1, wl2, num_iter)) / (
            self.compute_wl_2norm(wl1, num_iter) * self.compute_wl_2norm(wl2, num_iter))
