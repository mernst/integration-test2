...
def find_top_k_similar_program(repo_kernel_file, user_prog_graph_dot_file, graph_name, k, num_iter,
    ...
    result_program_list_with_score = sim.find_top_k_similar_graphs(user_prog_graph_dot_file,
                                                                   graph_name, k, num_iter,
                                                                   cluster_json)
...
def main():
    ...
    start = time.time()
    result_program_list = find_top_k_similar_program(repo_kernel_file, user_prog_graph_dot_file,
                                                     user_prog_graph_dot_file, top_k, 3,
                                                     cluster_json)
    end = time.time()
    print
