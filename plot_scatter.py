...
def compute_method_text_similarity(m1_full_str, m2_full_str, name_re, camel_re, stemmer):
    ...
    # (5):
    score = float(common_word_len) / (sum([len(w) for w in m1_stemmed_word_lst]) +
                                      sum([len(w) for w in m2_stemmed_word_lst]))
    return score
            ...
            if len(line) > 0 and line[-1] == ":":
                ...
                if linarr[0][-3:] == "dot":
                    ...
                    if count == 0:
                        ...
                        text_score = compute_method_text_similarity(current_method, similar_method,
                                                                    name_re, camel_re, stemmer)
...
def main():
    ...
    for proj in proj_lst:
        ...
        method_dict = parse_result_file(os.path.join(args.cluster, proj_result_file_name),
                                        dot_method_map)
        xs = []
        ys = []
        ...
        plot_scatter(xs, "semantic similarity", ys, "name similarity", os.path.join(fig_dir, proj),
                     proj + " : " + args.strategy)
        # correlation:
